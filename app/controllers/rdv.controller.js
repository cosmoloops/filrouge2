...
exports.findOnebyId = (req, res) => {
 Rdv.find({cle:req.params.cle},function(err,rdv){
            if (err){
                res.send(err);
			}
            res.json(rdv);
...
exports.findOneRdvtitre=(req, res) => {
 Rdv.find({titre:{ $regex: new RegExp("^" + req.params.titre.toLowerCase(), "i") }},function(err,rdv){
            if (err)
                res.send(err);
            res.json(rdv);
        });
...
exports.findOneRdvkey=(req, res) => {
 Rdv.find({cle:{ $regex: new RegExp("^" + req.params.cle.toLowerCase(), "i") }},function(err,rdv){
            if (err)
                res.send(err);
            res.json(rdv);
        });
...
exports.updateCle = (req, res) => {
	...
	Rdv.findOneAndUpdate({cle:{ $regex: new RegExp("^" + req.params.cle.toLowerCase(), "i") }}, {
		cle: req.body.cle, 
		titre: req.body.titre,
		type:req.body.type,
		dateDebut:req.body.dateDebut,
...
Rdv.findOneAndDelete({cle:req.params.cle})
	...
	.then(rdv => {
		...
		res.send({message: "cle Rdv deleted successfully!"});
	}).catch(err => {
...
Rdv.findByIdAndRemove(req.params.id)
	.then(rdv => {
		...
		res.send({message: "Rdv deleted successfully2!"});
	}).catch(err => {
		if(err.kind === 'ObjectId' || err.name === 'NotFound') {
			return res.status(404).send({
				message: "Rdv not found with id " + req.params.id
